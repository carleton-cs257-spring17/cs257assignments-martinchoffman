...
def get_max_city(state_name, city_name):
	...
	row = _fetch_all_rows_for_query(query)
	max = {'State': '{}'.format(state_name), 'City':'{}'.format(city_name), 
		   'Max Temp':row[0]}
	return max
...
def get_min_city(state_name, city_name):
	...
	row = _fetch_all_rows_for_query(query)
	min = {'State': '{}'.format(state_name), 'City':'{}'.format(city_name), 
		   'Min Temp':row[0]}
	return min
...
def get_mean_city(state_name, city_name):
	...
	mean = round((sum(mean_list)/len(mean_list)), 1)
	mean_city_dict = {'State': '{}'.format(state_name), 
...
def get_rainy_days_city(state_name, city_name):
	...
	rain_city_dict = {'State': '{}'.format(state_name), 
					  'City':'{}'.format(city_name), 
...
def get_snowy_days_city(state_name, city_name):
	...
	snow_city_dict = {'State': '{}'.format(state_name), 
					  'City':'{}'.format(city_name), 
	...
	for dictionary in city_list:
		...
		if (len(mean_list) != 0):
			...
			if mean >= temp1 and mean <= temp2:
				city_dict = {'State':'{}'.format(dictionary.get('state')),
							 'City': '{}'.format(city), 
							 'Mean Temp': mean}
...
def get_cities_days(temp1, temp2):
	...
	for dictionary in city_list:
		...
		if (len(mean_list) != 0) and (get_stations_by_city(dictionary.get('state'), 
									  dictionary.get('city')) != 0):
			days = int(len(mean_list)/(get_stations_by_city(dictionary.get('state'), 
									   dictionary.get('city'))))
